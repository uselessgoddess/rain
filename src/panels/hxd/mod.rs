...
use egui::{
  Context, Label, Margin, RichText, ScrollArea, Sense, TextEdit, TextWrapMode,
...
use self::data::BetweenFrameData;

pub use self::data::MemoryEditorOptions;
...
impl MemoryEditor {
  ...
  fn draw_editor_contents_impl<T: ?Sized>(
    ...
    let address_space =
      self.address_ranges.get(&selected_address_range).unwrap().clone();
    // This is janky, but can't think of a better way.
    let address_characters =
      format!("{:X}", address_space.end - 1).chars().count();
    ...
    if let Some(line) = self.frame_data.goto_address_line.take() {
      let new_offset =
        (line_height + ui.spacing().item_spacing.y) * (line as f32);
  ...
  fn draw_memory_values<T: ?Sized>(
    ...
    let mut read_only =
      frame_data.selected_edit_address.is_none() || write_fn.is_none();
    ...
    ui.horizontal(|ui| {
      ...
      ui.horizontal(|ui| {
        for i in 0..options.column_count {
          ...
          let character =
            if !(32..128).contains(&mem_val) { '.' } else { mem_val as char };
          let mut text = RichText::new(character)
            .text_style(options.memory_editor_ascii_text_style.clone());
  ...
  fn get_line_height(&self, ui: &mut Ui) -> f32 {
    let address_size =
      ui.text_style_height(&self.options.memory_editor_address_text_style);
    let body_size =
      ui.text_style_height(&self.options.memory_editor_text_style);
    let ascii_size =
      ui.text_style_height(&self.options.memory_editor_ascii_text_style);
  ...
  pub fn set_address_range(
    &mut self,
    range_name: impl Into<String>,
    address_range: Range<Address>,
  ) {
    self.address_ranges.insert(range_name.into(), address_range);
    self.frame_data.memory_range_combo_box_enabled =
      self.address_ranges.len() > 1;
