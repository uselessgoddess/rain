...
pub fn slice_to_decimal_string(
  ...
  match data_preview.selected_endianness {
    Endianness::Big => match data_preview.selected_data_format {
      DataFormatType::U8 => {
        u8::from_be_bytes(bytes.try_into().unwrap()).to_string()
      }
      DataFormatType::U16 => {
      ...
      DataFormatType::I8 => {
        i8::from_be_bytes(bytes.try_into().unwrap()).to_string()
      }
      DataFormatType::I16 => {
    ...
    Endianness::Little => match data_preview.selected_data_format {
      DataFormatType::U8 => {
        u8::from_le_bytes(bytes.try_into().unwrap()).to_string()
      }
      DataFormatType::U16 => {
      ...
      DataFormatType::I8 => {
        i8::from_le_bytes(bytes.try_into().unwrap()).to_string()
      }
      DataFormatType::I16 => {
      ...
      DataFormatType::F32 => {
        f32::from_le_bytes(bytes.try_into().unwrap()).to_string()
      }
      DataFormatType::F64 => {
        f64::from_le_bytes(bytes.try_into().unwrap()).to_string()
      }
    },
  }
}
