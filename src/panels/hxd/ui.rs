...
use super::{
  Address, MemoryEditor,
  data::{DataFormatType, DataPreviewOptions, Endianness},
};
...
impl MemoryEditor {
  ...
  fn draw_main_options(
    ...
    egui::Grid::new("options_grid").show(ui, |ui| {
      ...
      let response = ui
        ...
        .on_hover_text(
          "Goto an address, format: \n\
              ...
              * Offset from the base address, if the base is `0xFF00` \
              then one can enter `5` to go to `0xFF05`\n\
      ...
      if response.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter))
      {
        ...
        if goto_address_string.starts_with("0x")
          || goto_address_string.starts_with("0X")
        {
        ...
        let address = Address::from_str_radix(goto_address_string, 16)
          ...
          .and_then(|addr| {
            if current_address_range.contains(&addr) {
              ...
              // For brevity the user should be able to elide the base address,
              // e.g when using the range
              // 0xFF00..0xFFFF the user can write 0x5 to go to 0xFF05
              let offset_addr =
                addr.saturating_add(current_address_range.start);

              if current_address_range.contains(&offset_addr) {
                // We're doing an offset jump,
                // we should update the string to reflect the absolute address
  ...
  fn draw_data_preview<T: ?Sized>(
        ...
        egui::Grid::new("data_preview_grid").show(ui, |ui| {
          ...
          egui::ComboBox::from_label("Endianness")
            .selected_text(format!(
              "{:?}",
              data_preview_options.selected_endianness
            ))
            .show_ui(ui, |ui| {
          ...
          egui::ComboBox::from_label("Format")
            .selected_text(format!(
              "{:?}",
              data_preview_options.selected_data_format
            ))
            .show_ui(ui, |ui| {
          ...
          let hover_text = "Right click a value in the UI to select it,\
            right click again to unselect";
    ...
    // Currently relies on the header being open_default(false),
    // otherwise we'd enable the highlight when closing the preview!
    if response.header_response.clicked() {
      self.frame_data.show_additional_highlights =
        !self.frame_data.show_additional_highlights;
    }
  }
